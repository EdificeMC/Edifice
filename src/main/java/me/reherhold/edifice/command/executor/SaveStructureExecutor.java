...
public class SaveStructureExecutor implements CommandExecutor {
	...
	class SaveStructureRunnable implements Runnable {
		...
		public void run() {
			...
			Direction direction = Direction.getClosestHorizontal(convertRotation(player.getRotation()),
					Division.CARDINAL);

			JSONObject structure = new JSONObject().put(NAME, this.structureName)
					.put(CREATOR_UUID, this.player.getUniqueId().toString()).put(WIDTH, maxX - minX + 1)
			...
			default:
				player.sendMessage(
						Text.of(TextColors.RED, "Look in the direction facing the front of your structure."));
				return;
			}
			...
			if (response.getStatus() == 201) {
				...
				this.player.sendMessage(Text.of(TextColors.GREEN, "You have successfully uploaded ", TextColors.GOLD,
						this.structureName, TextColors.GREEN, "."));
				try {
